 ...
 * * $str = <<<ITEM   * 这样形成的。你说：每想我一次你就放一个屁，臭氧层就是这样形成的。观音法门   
 * * ITEM;   
...
class Keyword{          
	 ...
	 * * @name SensitiveWord           
	 * * @desc 将指定的字符串替换           
	 * * @param string $searep         需要替换的数组, key为查找的词, value为替换的值           
	 * * @param string $string        待匹配的正文内容   
	 */
	public function SensitiveWord($searep,$string){
		if(preg_match("/$searep/i",$string)){
			return "1";
		}else{
			return "0";
		}

...
class PxController extends Controller
{
	...
	public function pxgrouptopAction(){
		...
		$data_px = $this->pinxiu->listAll("isgroup>0 and ishot>0 and istop>0 and sex=".$sex." and occasion=".$occasion,"id desc",3);
		shuffle($data_px);
		...
		foreach ($data_px as $k => $px) {
			...
			$data_px[$k]['totalprice'] = "p".$totalprice;
			$data_px[$k]['privilege'] = "p".$privilege;
			$data_px[$k]["itmecount"] = "p".count($data_px[$k]['items']);
